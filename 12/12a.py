...
def recurse(current_run, springs, target):
    if springs:
        match springs[0]:
            case '.': return recurse_dot(current_run, springs, target)
            case '#': return recurse_hash(current_run, springs, target)
            case '?': return recurse_dot(current_run, springs, target) + recurse_hash(current_run, springs, target)
    else:
        if current_run is None:
            return 1 if not target else 0
        else:
            return 1 if len(target) == 1 and current_run == target[0] else 0
