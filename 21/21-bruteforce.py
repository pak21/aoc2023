...
def can_move(p, grid):
    return Action.BLOCKED if grid[p[1] % len(grid)][p[0] % len(grid[0])] else Action.MOVE
    
def part1(grid, todo, max_n):
    seen = {}
    right = []
    up = []
    left = []
...
def part1_with_min(grid, todo, max_n):
    min_moves = todo[0][0]
    corrected = [(n - min_moves, p) for n, p in todo]
    seen, right, up, left, down = part1(grid, corrected, max_n)
...
def main():
    ...
    seen, _, _, _, _ = part1_with_min(grid, [(0, start)], max_n)
